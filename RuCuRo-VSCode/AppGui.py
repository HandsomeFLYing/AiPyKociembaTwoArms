...
def save_picture():
    ...
    if not CAMARA_1:
        print("（图像处理 ：）模拟1")
        img = cv.imread('test/20250515_182018/33.84375.png')
        cube.img2points(img,"1")
        print("（图像处理 ：）模拟2")
        img = cv.imread('test/20250515_182018/43.90625.png')
        cube.img2points(img,"2")
        print("（图像处理 ：）模拟3")
        img = cv.imread('test/20250515_182018/38.859375.png')
...
def cube_cv_img():
    print("（图像处理 ：）Pi"+"\t"+str(time.process_time()))
    camera = cv.VideoCapture(0)
    ret, img = camera.read()
    img = cv.resize(img, (480,360), interpolation=cv.INTER_CUBIC)
    camera.release()
    #cube.img2points(img,str(0))
    cv.imwrite('./picture/'+str(time.process_time())+'.png',img)
